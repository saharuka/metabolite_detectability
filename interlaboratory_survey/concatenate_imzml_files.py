...
def concat_imzml_files(input_paths, offsets, input_mz_ranges, output_path, rot=0):
    parsers = []
    ...
    with ImzMLWriter(output_path) as writer:
        ...
        coords = np.array(list(output_mzs.keys()))
        max_x, max_y = coords.max(axis=0)
        for x, y in sorted(output_mzs.keys()):
            mzs = np.concatenate(output_mzs[(x,y)])
            ints = np.concatenate(output_ints[(x,y)])
            if(rot == -1): #CCW rot
                writer.addSpectrum(mzs, ints, (y+1, (max_x-x)+1, 1))
            elif (rot == 1): #CW rot
                writer.addSpectrum(mzs, ints, ((max_y-y)+1, x+1, 1))
            elif (rot == 11): #CW rot, flip along Y (Transposate)
                writer.addSpectrum(mzs, ints, (y+1, x+1, 1))
            elif (rot == -11):  # CCW rot, flip along Y (Reverse Transposate)
                    writer.addSpectrum(mzs, ints, ((max_y-y)+1, (max_x - x) + 1, 1))
            else:
                writer.addSpectrum(mzs, ints, (x + 1, y + 1, 1))

def transform_imzml(input_path, output_path, rot=0):
    print(f'Parsing imzML file for {input_path}')
    parser = ImzMLParser(str(input_path))

    # Read imzML file
    # Store in dict indexed by the output coordinate
    output_mzs = defaultdict(list)
    output_ints = defaultdict(list)
    print(f'Reading spectra for {input_path}')
    coords = np.array(parser.coordinates)[:, :2]
    ...
    with ImzMLWriter(output_path) as writer:
        ...
        for x, y in sorted(output_mzs.keys()):
            mzs = np.concatenate(output_mzs[(x,y)])
            ints = np.concatenate(output_ints[(x,y)])
            if(len(ints)==0): #I hate that this is necessary
                continue
            if(rot == -1): #CCW rot
                writer.addSpectrum(mzs, ints, (y+1, (max_x-x)+1, 1))
            elif (rot == 1): #CW rot
